...
**「光源が受光面と同じ高さにある」**という意味で、平らな面（法線は真上）への
入射角が 90 度になり、**何も照らされない。**

**ノーマルマップを渡したのに画面が暗くなったら、まずここを疑うこと。**
素材の問題ではない。

**単位は画素**（0〜1 の正規化値ではない）。実測した目安:

| height | 見え方 |
| --- | --- |
| 1〜16 | ほぼ真っ暗 |
| 64〜128 | **正しい。光溜まりが出て、凹凸も残る** |
| 256 以上 | 法線が効かなくなり、ノーマルマップ無しと同じになる |

**灯体の実際の高さを画素で書く。** 磐戸町の設定は
自販機の発光面 24 / 街灯 48 / 月光 96（`godot_preview/preview.gd`）。
