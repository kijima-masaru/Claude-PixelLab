...
`iwato_64.gpl` — GIMP / Aseprite 形式。`project.yaml` の `palette.file` から参照する。
...
| `green` 褪せた緑 | 8 | 夏枯れの草・杉・苔 |
| `earth` 土・錆・木 | 9 | 地面・トタン・木部・瓦 |
| `skin` 肌 | 4 | UI とアイコン用 |
| **光源色** | **12** | **ここだけ彩度を持つことが許される** |
| **合計** | **64** | |

**地の色は52色すべてが褪せている。** 彩度を持つのは光源12色だけである。
