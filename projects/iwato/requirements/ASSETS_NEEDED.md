...
高い単体物（objects）                     24         72                 未定  **未解決**
----------------------------------------------------------------------------------------
小計（未解決を除く）                        123        369          459 - 540

  タイルセットから得られるタイル: 432 枚（1本16タイル）

  ※ UI とアイコンは fields.json に含まれない。ASSETS_NEEDED.md を参照。
  ※ 高い単体物 24 点は作り方が未解決のため見積もらない。
```
...
**高さのある単体物 24点。** これが唯一の未解決領域です。
パイロットで `/map-objects`（アイソメ）と `/create-image-pixflux`（正面立面）の
両方が本作の視点に合わないことを実証しました。

24点のうち、**連続する構造物はタイルセットへ寄せられる可能性があります**
（防音壁、ブロック塀、金網、ガードレール、バックネット）。
**単体で立つ物**（自販機、電柱、街灯、鳥居、樹木）だけが本当に未解決です。

次の検証：`obj_vending_machine` を `/create-tileset` で作れるか（3〜4 generations）。
